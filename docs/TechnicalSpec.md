...
│   GLM-5.1 → DeepSeek-v4-pro → SenseNova 自动降级链                │
├─────────────────────────────────────────────────────────────────────┤
│                       基础服务层 (Services)                          │
...
│   AKShare/Tushare(数据源) · SQLite+Redis(存储) · Docker(部署)      │
...
| 基础设施层 | 数据存储、缓存、容器化部署 | SQLite、Redis、Docker |
...
| 类别 | 工具 | 数据源 |
|------|------|--------|
| 市场数据 | get_stock_quote, calc_technical_indicators, get_capital_flow, get_financial_data, get_sector_comparison | AKShare |
| 新闻舆情 | fetch_financial_news, fetch_announcements, analyze_sentiment, fetch_social_sentiment | 新闻API+jieba |
...
- LLM推理使用国产模型：GLM-5.1（智谱AI）、DeepSeek-v4-pro、SenseNova（商汤日日新）
- 所有模型API端点均为国内服务，数据不出境
- 市场数据通过AKShare/Tushare国内源获取，无跨境数据传输
...
**4. LLM三模型降级链：** GLM-5.1 → DeepSeek-v4-pro → SenseNova，指数退避重试。
